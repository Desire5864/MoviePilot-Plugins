...
class UhdBlurayAutoDownload(_PluginBase):
    ...
    def __fill_missing_subtitles(self, processed_map: Dict[str, Dict[str, Any]]) -> None:
        ...
        for record_key, record in processed_map.items():
            ...
            # 补齐种子标题（站点详情页「下载」字段）
            if not record.get("qb_name"):
                seed_name = self.__fetch_detail_seed_name(site, torrent_id)
                if seed_name:
    ...
    def __fetch_detail_seed_name(self, site: Dict[str, Any], torrent_id: str) -> str:
        """从种子详情页的「下载」字段获取种子文件名。

        站点详情页「下载」字段的链接文本即为种子文件名
        （形如「[CHDBits].惊声尖笑6.Scary.Movie.2026...torrent」），
        去掉站点前缀与 .torrent 后缀后即为 QB 任务名，
        供 BDMV自动打包ISO 插件精确匹配。

        :param site: 站点配置
        :param torrent_id: 种子 ID
        :return: 种子标题；获取失败返回空字符串
        """
        if not torrent_id:
            return ""
        # 命中缓存直接返回，避免重复请求详情页
        cache_key = f"seed:{torrent_id}"
        ...
        for attempt in range(2):
            ...
            if name:
                self._subtitle_cache[cache_key] = name
                return name
            return ""

        logger.warning(f"UHD原盘自动下载：详情页种子标题获取失败，已重试 {detail_url}")
        return ""
    ...
    def __find_qb_torrent_name(downloader_obj: Any, title: str) -> str:
        ...
        for torrent in torrents or []:
            name = getattr(torrent, "name", None) or ""
            if not name:
                continue
            norm_name = normalize(name)
            # 归一化全等或互相包含（QB 任务名可能带中文前缀）
            if norm_name == target or target in norm_name or norm_name in target:
                return str(name)
            # 关键词重合匹配（兼容版本标记差异）
            if target_tokens:
                rec_tokens = tokens(name)
                if rec_tokens:
